...
class UrlTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $urlObject = new Url('https://www.google.com/truc-much/test/?rien=46&test=4#page-anchor');

        $this->assertEquals($urlObject->getUrl(), 'https://www.google.com/truc-much/test/?rien=46&test=4#page-anchor');
        $this->assertEquals($urlObject->getHost(), 'www.google.com');
        $this->assertEquals($urlObject->getDomain(), 'google.com');
        $this->assertEquals($urlObject->getProtocol(), 'https');
        $this->assertEquals($urlObject->getHashtag(), 'page-anchor');
        $this->assertEquals($urlObject->getSchemeAndHttpHost(), 'https://www.google.com');
    }
    ...
    public function testNormalizeLink()
    {
        ...
        $urls = array(
            ...
            '/jcms/c_2055287/',
            'http://test.com#yo',
            'http://video.youtube.co.uk/test-url',
            'jcms/c_2055287/',
        ...
        $this->assertEquals((new Url($urls[0]))->isProtocolLess(), false);
        $this->assertEquals((new Url($urls[0]))->isRelative(), true);

        $this->assertEquals((new Url($urls[0]))->getPath(), '/jcms/c_2055287/');
        $this->assertEquals((new Url($urls[1]))->getPath(), '/jcms/c_2055287/');

        $this->assertEquals((new Url($urls[2]))->hasAnchor(), true);

        $this->assertEquals((new Url($urls[3]))->getDomain(), 'youtube.co.uk');

        // foreach ($assertions as $urlInput => $mustEqual) {
